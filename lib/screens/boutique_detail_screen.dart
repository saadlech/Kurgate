...
class _BoutiqueDetailScreenState extends State<BoutiqueDetailScreen> {
          ...
          SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 20, 20, 120), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(_boutique.name, style: const TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white, fontSize: 28, fontWeight: FontWeight.w800, letterSpacing: -0.3)),
                const SizedBox(height: 4),
                Row(children: [Icon(Icons.person_rounded, color: Colors.white.withValues(alpha: 0.4), size: 15), const SizedBox(width: 4), Text(_boutique.artisan, style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.5), fontSize: 14, fontWeight: FontWeight.w500))]),
                const SizedBox(height: 2),
                Row(children: [Icon(Icons.location_on_outlined, color: Colors.white.withValues(alpha: 0.4), size: 15), const SizedBox(width: 4), Expanded(child: Text(_boutique.location, style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.4), fontSize: 14), overflow: TextOverflow.ellipsis))]),
              ])),
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFFF8C00).withValues(alpha: 0.15), borderRadius: BorderRadius.circular(10)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.star_rounded, color: Color(0xFFFF8C00), size: 18), const SizedBox(width: 4), Text(_boutique.rating.toString(), style: const TextStyle(fontFamily: 'DarkerGrotesque', color: Color(0xFFFF8C00), fontSize: 15, fontWeight: FontWeight.w800)), Text(' (${_boutique.reviews})', style: const TextStyle(fontFamily: 'DarkerGrotesque', color: Color(0xFFFF8C00), fontSize: 12))])),
            ]),
            const SizedBox(height: 20),
            Text(_boutique.description, style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.6), fontSize: 14, height: 1.6)),
            const SizedBox(height: 24),
            Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              _buildSpec(Icons.storefront_rounded, _boutique.category), _buildSpec(Icons.access_time_rounded, _boutique.horaires),
              _buildSpec(Icons.handshake_rounded, 'Fait main'), _buildSpec(Icons.local_shipping_rounded, 'Livraison'),
            ]),
            const SizedBox(height: 24),
            // Products
            const Text('Produits', style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
            const SizedBox(height: 12),
            ...List.generate(_boutique.products.length, (i) {
              final p = _boutique.products[i]; final active = i == _selectedProduct;
              return GestureDetector(onTap: () => setState(() { _selectedProduct = i; _quantity = 1; }),
                child: AnimatedContainer(duration: const Duration(milliseconds: 200), margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(14),
                  decoration: BoxDecoration(color: active ? Colors.white.withValues(alpha: 0.06) : Colors.transparent, borderRadius: BorderRadius.circular(12), border: Border.all(color: active ? const Color(0xFFFF8C00).withValues(alpha: 0.5) : Colors.white.withValues(alpha: 0.06))),
                  child: Row(children: [
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(p.name, style: TextStyle(fontFamily: 'DarkerGrotesque', color: active ? Colors.white : Colors.white.withValues(alpha: 0.6), fontSize: 15, fontWeight: FontWeight.w700)),
                      Text(p.desc, style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.3), fontSize: 12)),
                    ])),
                    Text('\$${p.price}', style: TextStyle(fontFamily: 'DarkerGrotesque', color: active ? const Color(0xFFFF8C00) : Colors.white.withValues(alpha: 0.5), fontSize: 17, fontWeight: FontWeight.w800)),
                    if (active) ...[const SizedBox(width: 8), const Icon(Icons.check_circle_rounded, color: Color(0xFFFF8C00), size: 20)],
                  ])));
            }),
            const SizedBox(height: 20),
            // Quantity
            Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.03), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.white.withValues(alpha: 0.06))),
              child: Row(children: [
                const Expanded(child: Text('Quantité', style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white, fontSize: 15, fontWeight: FontWeight.w700))),
                _counterBtn(Icons.remove_rounded, _quantity > 1, () => setState(() => _quantity--)),
                Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Text('$_quantity', style: const TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700))),
                _counterBtn(Icons.add_rounded, _quantity < 10, () => setState(() => _quantity++)),
              ])),
            ...
            Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.04), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.white.withValues(alpha: 0.06))),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 12),
                _summaryRow('Article', _currentProduct.name), _summaryRow('Prix unitaire', '\$${_currentProduct.price}'), _summaryRow('Quantité', '$_quantity'),
        ...
        Positioned(left: 0, right: 0, bottom: 0, child: Container(
          ...
          child: Row(children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              ...
              Text('$_quantity × ${_currentProduct.name}', style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.35), fontSize: 12), overflow: TextOverflow.ellipsis),
            ]),
            const Spacer(),
            GestureDetector(onTap: _showOrderConfirmation, child: Container(height: 48, width: 160,
              decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFF8C00), Color(0xFFE77728)]), borderRadius: BorderRadius.circular(30), boxShadow: [BoxShadow(color: const Color(0xFFFF8C00).withValues(alpha: 0.3), blurRadius: 16, offset: const Offset(0, 6))]),
              child: const Center(child: Text('Commander', style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.black, fontSize: 16, fontWeight: FontWeight.w700))))),
          ]),
        )),
      ]),
    );
  ...
  Widget _buildSpec(IconData icon, String label) => Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    const SizedBox(height: 6), SizedBox(width: 70, child: Text(label, style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.4), fontSize: 11, fontWeight: FontWeight.w500), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis)),
  ]);
...
class _OrderSheetState extends State<_OrderSheet> with SingleTickerProviderStateMixin {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (_confirmed) ...[
          ...
          Text('Votre commande chez ${widget.boutiqueName} est enregistrée.', textAlign: TextAlign.center, style: TextStyle(fontFamily: 'DarkerGrotesque', color: Colors.white.withValues(alpha: 0.5), fontSize: 14)),
          const SizedBox(height: 24),
        ] else ...[
          ...
          Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.04), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.white.withValues(alpha: 0.08))),
            child: Column(children: [
              _row(Icons.storefront_rounded, widget.boutiqueName), _row(Icons.shopping_bag_rounded, '${widget.productName} × ${widget.quantity}'),
